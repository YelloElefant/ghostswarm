...
const MAX_CHUNK_SIZE = 32 * 1024; // 32KB chunks
const MAX_CONCURRENT_REQUESTS = 5; // Limit concurrent requests per peer
...
// Track active connections with better memory management
const connectedPeers = new Map();
const activeTorrents = new Map(); // infoHash -> Set of peer IDs that have it
const activeRequests = new Map(); // Track active requests per peer
...
async function handleAnnounce(peerId, message) {
   ...
   const swarmPeers = Array.from(activeTorrents.get(infoHash) || [])
      ...
      .slice(0, 20) // Limit to 20 peers max
      .map(id => ({
         peerId: id,
...
async function handleGetPeers(peerId, message) {
   ...
   const swarmPeers = Array.from(activeTorrents.get(infoHash) || [])
      .filter(id => id !== peerId && connectedPeers.has(id))
      .slice(0, 10) // Limit response size
      .map(id => ({
         peerId: id,
         ip: connectedPeers.get(id).ip,
...
app.get('/stats', (req, res) => {
   ...
   for (const [infoHash, peers] of activeTorrents.entries()) {
      if (torrentCount >= 50) break; // Limit to 50 torrents

      stats.torrents[infoHash] = {
         swarmSize: peers.size,
         peers: Array.from(peers).slice(0, 10) // Limit to 10 peers per torrent
      };
      torrentCount++;
   }
...
setInterval(() => {
   ...
   // Log memory usage
   const memUsage = process.memoryUsage();
   console.log(`💾 Bot Memory: ${Math.round(memUsage.heapUsed / 1024 / 1024)}MB used, ${Math.round(memUsage.heapTotal / 1024 / 1024)}MB total, ${connectedPeers.size} peers`);

}, CLEANUP_INTERVAL);

// Graceful shutdown
process.on('SIGTERM', () => {
   console.log('🛑 Shutting down bot seeder...');
   ...
   wss.close(() => {
      console.log('✅ Bot seeder shutdown complete');
      process.exit(0);
   });
});

server.listen(PORT, () => {
   console.log(`🚀 Bot WebSocket seeding server running on port ${PORT}`);
